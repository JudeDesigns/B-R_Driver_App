...
# B&R Driver App - VPS Setup Script
# Run this once on your VPS to set up the deployment script
...
print_status "🚀 Setting up B&R Driver App deployment on VPS..."

# Set server timezone to PST for consistent date handling
...
if [ "$CURRENT_TZ" = "America/Los_Angeles" ]; then
    ...
    print_warning "⚠️  Timezone setting may have failed. Current: $CURRENT_TZ"
fi

# Create the deployment script on VPS
cat > /opt/deploy-br-app.sh << 'EOF'
#!/bin/bash
...
chmod +x /opt/deploy-br-app.sh

print_success "✅ Deployment script created at: /opt/deploy-br-app.sh"

# Create a simple alias for easy access
echo 'alias deploy-br="sudo /opt/deploy-br-app.sh"' >> ~/.bashrc

print_success "✅ Created alias 'deploy-br' for easy deployment"

print_status "🎉 Setup completed!"
echo ""
print_status "=== How to Use ==="
print_status "1. To deploy your app: sudo /opt/deploy-br-app.sh"
print_status "2. Or use the alias: deploy-br"
print_status "3. The script will automatically:"
print_status "   - Pull latest code from GitHub"
print_status "   - Create backups"
print_status "   - Build the application"
print_status "   - Restart PM2 with zero downtime"
print_status "   - Run health checks"
print_status "   - Rollback if anything fails"
echo ""
print_success "🚀 Your VPS is now ready for automated deployments!"
